...
def process_files(chatteurs_path, creator_path, output_dir):
    ...
    for index, row in df_chat.iterrows():
        ...
        result = {
        # Complément de tous les KPIs attendus par le template PDF
        result.update({
            ...
            "Messages / fan": safe_divide(row.get("Messages sent", 0), row.get("Fans chatted", 1)),
            ...
            "Keystrokes / msg": round(row.get("Keystrokes (words)", 0) / row.get("Messages sent", 1), 2) if row.get("Messages sent", 0) > 0 else None, 0) / row.get("Messages sent", 1),
