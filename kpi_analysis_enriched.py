...
def process_files(chatteurs_path, creator_path, output_dir):
    ...
    for index, row in df_chat.iterrows():
        ...
        creator_row = df_creator[df_creator["Group"] == group]
        if not creator_row.empty:
            ...
            try:
                ...
                fans_modele = 0
            try:
                subs_modele = int(creator_row["New subs"].values[0])
            except:
                subs_modele = 0
        else:
            ca_modele = 0.0
        ...
        result = {
            ...
            "ratio_clocksched": safe_divide(row["Clocked minutes"], row["Scheduled minutes"]) * 100
        }
