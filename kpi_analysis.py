...
def generate_pdf(result, output_dir):
    ...
    try:
        ...
        pdf_path = os.path.join(output_dir, f"{result['chatteur']
        "Reply time": row.get("Reply time"),
        "sales": round(result["salaire_net"] / 0.15, 2) if result.get("salaire_net") else 0,
        "new_subs": creator_row.get("New subscriptions Net") if creator_row is not None else None,
        "tips_net": creator_row.get("Tips Net") if creator_row is not None else None,
        "messages_net": creator_row.get("Message Net") if creator_row is not None else None,
        "earnings_net": creator_row.get("Total earnings Net") if creator_row is not None else None,
