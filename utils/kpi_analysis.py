...
def generate_chatters_json(inflow_file, creator_file):
    ...
    for _, row in inflow_df.iterrows():
        ...
        if ppv_envoyes < 10: flags.append("Peu de PPV envoyés")
        ...
        if unlock_ratio < 0.1 and ppv_envoyes > 20: typologies.append("Push dans le vide")
        ...
        # Lecture Creator Statistics
        creator_row = creator_df[creator_df['Modèle'] == modele].squeeze()
        ca_modele = creator_row.get('Sales NET', 0) if not creator_row.empty else 0
        nb_fans = creator_row.get('Nombre de fans', 0) if not creator_row.empty else 0
        ratio_vs_modele = round(sales / ca_modele, 3) if ca_modele else 0
        ...
        results.append({
            ...
            "modele_stats": {
                "ca_total_modele": ca_modele,
                "nombre_fans": nb_fans,
                "ratio_ca_chatteur_vs_modele": ratio_vs_modele
            }
        })
